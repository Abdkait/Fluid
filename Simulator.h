    ...
    while (std::getline(file, line)) {
        ...
        if (line.find("\"g\"") != std::string::npos) {
            ...
            while (std::getline(file, line)) {
                ...
                for(int i = 0; i < M; ++i){
                    field_[count][i] = line[i+1];
                }
                ++count;
            }
            if(count != N){
                std::cerr << "Ошибка: количество строк в файле не соответствует запросу S(N, M)" << std::endl;
                exit(1);
            }
        }
    }
...
template<typename Ptype, typename VType, typename VFlowType, size_t N, size_t M>
void Simulator<Ptype, VType, VFlowType, N, M>::runSimulation(size_t T, size_t save_interval, const std::string& file_name)
{
    if (file_name.size() == 0) {
        readInputFile("../fluid.json");
    } else {
        readInputFile(file_name);
    }

    if (rho_[' '] == 0 || g_ == 0) {
        std::cout << "плотность воздуха и g не могут быть 0\n";
        return;
    } else if (rho_['.'] <= 0) {
        std::cout << "неположительная плотность воды?)\n";
        return;
    }
